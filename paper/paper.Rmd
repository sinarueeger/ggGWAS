...
```{r qqplot-example-hex-zoom, fig.height=6, fig.width=6, out.width="40%"}
qp <- ggplot(data = data_height) + 
  geom_gwas_qq_hex(aes(y = Pvalue), bins = 100, hex.function = ggplot2:::hexBinSummarise) + 
  geom_abline(intercept = 0, slope = 1, linetype = 2)

qp + coord_cartesian(ylim = c(0, -log10(0.05)), xlim = c(0, -log10(0.05)))

```


```{r qqplot-example-group, fig.height=6, fig.width=6}
ggplot(data = data_height) + 
  geom_gwas_qq(aes(y = Pvalue)) + 
...
```{r qqplot-example-scale, fig.height=6, fig.width=6}
ggplot(data = data_height) + 
